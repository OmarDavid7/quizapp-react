...
export default function Results({filtroPreguntas, score, onReset}) {
  return(
    <>
      <div className="flex flex-col justify-evenly items-center shadow-xl rounded-lg w-[600px] h-[600px] gap-5">
                ...
                <div className="flex flex-col gap-5 text-center text-lg font-bold">
                    ...
                    text-transparent text-6xl animate-pulse">
                        {((score / filtroPreguntas.length) * 100 ).toFixed(0)}%
                    </span>
                    de las preguntas ({score} de {filtroPreguntas.length})
                </div>

                <button className="border px-5 py-2 rounded-lg transition-all font-bold
                hover:bg-yellow-500 hover:text-gray-900" onClick={onReset}>Vamos de nuevo</button>
